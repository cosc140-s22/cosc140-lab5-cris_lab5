...
class Product(models.Model):
    name = models.CharField(max_length=50, blank=False,validators=[MaxLengthValidator(6)])
    description = models.TextField(blank=True)
    price = models.DecimalField(max_digits=6, decimal_places=2, blank=False,validators=[MaxLengthValidator(6)])
    minimum_age_appropriate = models.IntegerField(default=0, blank=False,validators=[MinValueValidator(1)])
    maximum_age_appropriate = models.IntegerField(default=-1, blank=False)
    release_date = models.DateField(validators=[MaxLengthValidator(10)])
